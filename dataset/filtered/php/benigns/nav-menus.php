...
?><?php esc_html_e( 'Menus' ); ?><?php
	...
	?><?php esc_attr_e( 'Secondary menu' ); ?><?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?><?php echo $nav_tab_active_class; ?><?php echo $nav_aria_current; ?><?php esc_html_e( 'Edit Menus' ); ?><?php
		if ( $num_locations && $menu_count ) {
			...
			?><?php echo esc_url( add_query_arg( array( 'action' => 'locations' ), admin_url( 'nav-menus.php' ) ) ); ?><?php echo $active_tab_class; ?><?php echo $aria_current; ?><?php esc_html_e( 'Manage Locations' ); ?><?php
		}
		?><?php
	...
	if ( $locations_screen ) :
		...
		?><?php echo esc_url( add_query_arg( array( 'action' => 'locations' ), admin_url( 'nav-menus.php' ) ) ); ?><?php _e( 'Theme Location' ); ?><?php _e( 'Assigned Menu' ); ?><?php foreach ( $locations as $_location => $_name ) { ?><?php echo $_location; ?><?php echo $_name; ?><?php echo $_location; ?><?php echo $_location; ?><?php printf( '&mdash; %s &mdash;', esc_html__( 'Select a Menu' ) ); ?><?php
								foreach ( $nav_menus as $menu ) :
									...
									?><?php echo $data_orig; ?><?php selected( $selected ); ?><?php echo $menu->term_id; ?><?php echo wp_html_excerpt( $menu->name, 40, '&hellip;' ); ?><?php endforeach; ?><?php if ( isset( $menu_locations[ $_location ] ) && 0 !== $menu_locations[ $_location ] ) : ?><?php
									echo esc_url(
										add_query_arg(
											array(
												'action' => 'edit',
									...
									?><?php _ex( 'Use new menu', 'menu' ); ?><?php } // End foreach. ?><?php submit_button( __( 'Save Changes' ), 'primary left', 'nav-menu-locations', false ); ?><?php wp_nonce_field( 'save-menu-locations' ); ?><?php echo esc_attr( $nav_menu_selected_id ); ?><?php
		/**
		...
		?><?php else : ?><?php if ( $menu_count < 1 ) : ?><?php _e( 'Create your first menu below.' ); ?><?php
				/* translators: Hidden accessibility text. */
				_e( 'Fill in the Menu Name and click the Create Menu button to create your first menu.' );
				?><?php elseif ( $menu_count < 2 ) : ?><?php
			printf(
			...
			?><?php
				...
				?><?php else : ?><?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?><?php _e( 'Select a menu to edit:' ); ?><?php if ( $add_new_screen ) : ?><?php _e( '&mdash; Select &mdash;' ); ?><?php endif; ?><?php foreach ( (array) $nav_menus as $_nav_menu ) : ?><?php echo esc_attr( $_nav_menu->term_id ); ?><?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?><?php
						...
						?><?php endforeach; ?><?php esc_attr_e( 'Select' ); ?><?php
				printf(
					/* translators: %s: URL to create a new menu. */
		...
		?><?php echo $metabox_holder_disabled_class; ?><?php echo esc_attr( $nav_menu_selected_id ); ?><?php wp_nonce_field( 'add-menu_item', 'menu-settings-column-nonce' ); ?><?php _e( 'Add menu items' ); ?><?php do_accordion_sections( 'nav-menus', 'side', null ); ?><?php _e( 'Menu structure' ); ?><?php
					...
					?><?php echo esc_attr( $nav_menu_selected_id ); ?><?php _e( 'Menu Name' ); ?><?php echo $menu_name_val . $menu_name_aria_desc; ?><?php submit_button( empty( $nav_menu_selected_id ) ? __( 'Create Menu' ) : __( 'Save Menu' ), 'primary large menu-save', 'save_menu', false, array( 'id' => 'save_menu_header' ) ); ?><?php if ( ! $add_new_screen ) : ?><?php
								$hide_style = '';
								...
								?><?php echo $hide_style; ?><?php echo $starter_copy; ?><?php if ( ! $add_new_screen ) : ?><?php echo $hide_style; ?><?php _e( 'Bulk Select' ); ?><?php endif; ?><?php
								if ( isset( $edit_markup ) && ! is_wp_error( $edit_markup ) ) {
									echo $edit_markup;
								} else {
									?><?php } ?><?php endif; ?><?php if ( $add_new_screen ) : ?><?php _e( 'Give your menu a name, then click Create Menu.' ); ?><?php if ( isset( $_GET['use-location'] ) ) : ?><?php echo esc_attr( $_GET['use-location'] ); ?><?php endif; ?><?php
							endif;
							...
							?><?php if ( ! $add_new_screen ) : ?><?php echo $hide_style; ?><?php _e( 'Bulk Select' ); ?><?php esc_attr_e( 'Remove Selected Items' ); ?><?php _e( 'List of menu items selected for deletion:' ); ?><?php endif; ?><?php echo $no_menus_style; ?><?php _e( 'Menu Settings' ); ?><?php
								if ( ! isset( $auto_add ) ) {
								...
								?><?php _e( 'Auto add pages' ); ?><?php checked( $auto_add ); ?><?php printf( __( 'Automatically add new top-level pages to this menu' ), esc_url( admin_url( 'edit.php?post_type=page' ) ) ); ?><?php if ( current_theme_supports( 'menus' ) ) : ?><?php _e( 'Display location' ); ?><?php
										foreach ( $locations as $location => $description ) :
											$checked = false;
											...
											?><?php checked( $checked ); ?><?php echo esc_attr( $location ); ?><?php echo esc_attr( $location ); ?><?php echo esc_attr( $nav_menu_selected_id ); ?><?php echo esc_attr( $location ); ?><?php echo $description; ?><?php if ( ! empty( $menu_locations[ $location ] ) && $menu_locations[ $location ] !== $nav_menu_selected_id ) : ?><?php
														printf(
															/* translators: %s: Menu name. */
															_x( '(Currently set to: %s)', 'menu location' ),
															wp_get_nav_menu_object( $menu_locations[ $location ] )->name
														);
													?><?php endif; ?><?php endforeach; ?><?php endif; ?><?php if ( $menu_count > 0 ) : ?><?php if ( $add_new_screen ) : ?><?php echo esc_url( admin_url( 'nav-menus.php' ) ); ?><?php _e( 'Cancel' ); ?><?php else : ?><?php
									echo esc_url(
										wp_nonce_url(
											add_query_arg(
												array(
									...
									?><?php _e( 'Delete Menu' ); ?><?php endif; ?><?php endif; ?><?php submit_button( empty( $nav_menu_selected_id ) ? __( 'Create Menu' ) : __( 'Save Menu' ), 'primary large menu-save', 'save_menu', false, array( 'id' => 'save_menu_footer' ) ); ?><?php endif; ?><?php require_once ABSPATH . 'wp-admin/admin-footer.php'; ?>