 ...
 */
do_action( "{$taxonomy}_pre_edit_form", $tag, $taxonomy ); ?><?php echo $tax->labels->edit_item; ?><?php
$class = ( isset( $msg ) && 5 === $msg ) ? 'error' : 'success';

if ( $message ) {
	?><?php echo $class; ?><?php echo $message; ?><?php if ( $wp_http_referer ) { ?><?php echo esc_url( wp_validate_redirect( sanitize_url( $wp_http_referer ), admin_url( 'term.php?taxonomy=' . $taxonomy ) ) ); ?><?php echo esc_html( $tax->labels->back_to_items ); ?><?php } ?><?php
}
?><?php
/**
...
?><?php echo esc_attr( $tag_ID ); ?><?php echo esc_attr( $taxonomy ); ?><?php
...
?><?php _ex( 'Name', 'term name' ); ?><?php echo $tag_name_value; ?><?php echo $tax->labels->name_field_description; ?><?php _e( 'Slug' ); ?><?php
			/**
			...
			?><?php echo esc_attr( $slug ); ?><?php echo $tax->labels->slug_field_description; ?><?php if ( is_taxonomy_hierarchical( $taxonomy ) ) : ?><?php echo esc_html( $tax->labels->parent_item ); ?><?php
				$dropdown_args = array(
					'hide_empty'       => 0,
					'hide_if_empty'    => false,
					'taxonomy'         => $taxonomy,
				...
				?><?php if ( 'category' === $taxonomy ) : ?><?php _e( 'Categories, unlike tags, can have a hierarchy. You might have a Jazz category, and under that have children categories for Bebop and Big Band. Totally optional.' ); ?><?php else : ?><?php echo $tax->labels->parent_field_description; ?><?php endif; ?><?php endif; // is_taxonomy_hierarchical() ?><?php _e( 'Description' ); ?><?php echo $tag->description; // textarea_escaped ?><?php echo $tax->labels->desc_field_description; ?><?php
		// Back compat hooks.
		if ( 'category' === $taxonomy ) {
			/**
...
?><?php submit_button( __( 'Update' ), 'primary', null, false ); ?><?php if ( current_user_can( 'delete_term', $tag->term_id ) ) : ?><?php echo esc_url( admin_url( wp_nonce_url( "edit-tags.php?action=delete&taxonomy=$taxonomy&tag_ID=$tag->term_id", 'delete-tag_' . $tag->term_id ) ) ); ?><?php _e( 'Delete' ); ?><?php endif; ?><?php if ( ! wp_is_mobile() ) : ?>