...
if ( $wp_customize->changeset_post_id() ) {
	...
	if ( $missed_schedule ) {
		...
		ob_start();
		?><?php wp_print_scripts( array( 'wp-util' ) ); ?><?php echo wp_json_encode( $request_args ); ?><?php
		$script = ob_get_clean();
...
?><?php echo esc_html( $admin_title ); ?><?php echo wp_json_encode( admin_url( 'admin-ajax.php', 'relative' ) ); ?><?php
/**
...
?><?php echo esc_attr( $body_class ); ?><?php
			...
			?><?php if ( $compatible_wp && $compatible_php ) : ?><?php $save_text = $wp_customize->is_theme_active() ? __( 'Publish' ) : __( 'Activate &amp; Publish' ); ?><?php submit_button( $save_text, 'primary save', 'save', false ); ?><?php esc_attr_e( 'Publish Settings' ); ?><?php else : ?><?php $save_text = _x( 'Cannot Activate', 'theme' ); ?><?php esc_attr_e( 'Publish Settings' ); ?><?php echo $save_text; ?><?php endif; ?><?php _e( 'Customize' ); ?><?php _e( 'Preview' ); ?><?php echo esc_url( $wp_customize->get_return_url() ); ?><?php
					/* translators: Hidden accessibility text. */
					_e( 'Close the Customizer and go back to the previous page' );
					?><?php // Outer panel and sections are not implemented, but its here as a placeholder to avoid any side-effect in api.Section. ?><?php echo (int) wp_is_block_theme(); ?><?php
						...
						?><?php
							...
							?><?php // Panels and sections are managed here via JavaScript ?><?php echo esc_attr_x( 'Hide Controls', 'label for hide controls button without length constraints' ); ?><?php _ex( 'Hide Controls', 'short (~12 characters) label for hide controls button' ); ?><?php $previewable_devices = $wp_customize->get_previewable_devices(); ?><?php if ( ! empty( $previewable_devices ) ) : ?><?php foreach ( (array) $previewable_devices as $device => $settings ) : ?><?php
						...
						?><?php echo esc_attr( $class ); ?><?php echo esc_attr( $active ); ?><?php echo esc_attr( $device ); ?><?php echo esc_html( $settings['label'] ); ?><?php endforeach; ?><?php endif; ?><?php
