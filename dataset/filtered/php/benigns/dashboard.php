...
function wp_dashboard() {
	...
	?><?php echo $columns_css; ?><?php do_meta_boxes( $screen->id, 'normal', '' ); ?><?php do_meta_boxes( $screen->id, 'side', '' ); ?><?php do_meta_boxes( $screen->id, 'column3', '' ); ?><?php do_meta_boxes( $screen->id, 'column4', '' ); ?><?php
...
function wp_dashboard_right_now() {
	...
	if ( $num_comm && ( $num_comm->approved || $num_comm->moderated ) ) {
		...
		?><?php echo ! $num_comm->moderated ? ' hidden' : ''; ?><?php echo $text; ?><?php
	}
...
function wp_network_dashboard_right_now() {
	...
	?><?php echo esc_url( network_admin_url( 'users.php' ) ); ?><?php
				/* translators: Hidden accessibility text. */
				_e( 'Search Users' );
				?><?php submit_button( __( 'Search Users' ), '', false, false, array( 'id' => 'submit_users' ) ); ?><?php echo esc_url( network_admin_url( 'sites.php' ) ); ?><?php
				/* translators: Hidden accessibility text. */
				_e( 'Search Sites' );
				?><?php submit_button( __( 'Search Sites' ), '', false, false, array( 'id' => 'submit_sites' ) ); ?><?php
	/**
...
function wp_dashboard_quick_press( $error_msg = false ) {
	...
	$post_ID = (int) $post->ID;
	?><?php echo esc_url( admin_url( 'post.php' ) ); ?><?php if ( $error_msg ) : ?><?php echo $error_msg; ?><?php endif; ?><?php
				/** This filter is documented in wp-admin/edit-form-advanced.php */
				echo apply_filters( 'enter_title_here', __( 'Title' ), $post );
				?><?php _e( 'Content' ); ?><?php esc_attr_e( 'What&#8217;s on your mind?' ); ?><?php echo $post_ID; ?><?php wp_nonce_field( 'add-post' ); ?><?php submit_button( __( 'Save Draft' ), 'primary', 'save', false, array( 'id' => 'save-post' ) ); ?><?php
...
function _wp_dashboard_recent_comments_row( &$comment, $show_date = true ) {
	...
	?><?php echo $comment->comment_ID; ?><?php comment_class( array( 'comment-item', wp_get_comment_status( $comment ) ), $comment ); ?><?php
			$comment_row_class = '';
			...
			?><?php if ( ! $comment->comment_type || 'comment' === $comment->comment_type ) : ?><?php echo $comment_row_class; ?><?php
				...
				?><?php comment_author_link( $comment ); ?><?php endif; // comment_type ?><?php comment_excerpt( $comment ); ?><?php if ( $actions_string ) : ?><?php echo $actions_string; ?><?php endif; ?><?php
...
function wp_print_community_events_markup() {
	?><?php _e( 'This widget requires JavaScript.' ); ?><?php _e( 'An error occurred. Please try again.' ); ?><?php _e( 'Loading&hellip;' ); ?><?php
	/*
	 * Hide the main element when the page first loads, because the content
	 * won't be ready until wp.communityEvents.renderEventsTemplate() has run.
	 */
	?><?php _e( 'Select location' ); ?><?php echo esc_url( admin_url( 'admin-ajax.php' ) ); ?><?php _e( 'City:' ); ?><?php
				 ...
				 */
				?><?php esc_attr_e( 'Cincinnati' ); ?><?php submit_button( __( 'Submit' ), 'secondary', 'community-events-submit', false ); ?><?php _e( 'Cancel' ); ?><?php
}
...
function wp_dashboard_php_nag() {
	...
	?><?php echo $message; ?><?php _e( 'What is PHP and how does it affect my site?' ); ?><?php _e( 'PHP is one of the programming languages used to build WordPress. Newer versions of PHP receive regular security updates and may increase your site&#8217;s performance.' ); ?><?php
...
function wp_dashboard_site_health() {
	...
	?><?php if ( false === $get_issues ) : ?><?php _e( 'No information yet&hellip;' ); ?><?php else : ?><?php _e( 'Results are still loading&hellip;' ); ?><?php endif; ?><?php if ( false === $get_issues ) : ?><?php
					printf(
...
function wp_welcome_panel() {
	...
	?><?php echo file_get_contents( dirname( __DIR__ ) . '/images/dashboard-background.svg' ); ?><?php _e( 'Welcome to WordPress!' ); ?><?php echo esc_url( admin_url( 'about.php' ) ); ?><?php
				/* translators: %s: Current WordPress version. */
				printf( __( 'Learn more about the %s version.' ), $display_version );
			?><?php _e( 'Author rich content with blocks and patterns' ); ?><?php _e( 'Block patterns are pre-configured block layouts. Use them to get inspired or create new pages in a flash.' ); ?><?php echo esc_url( admin_url( 'post-new.php?post_type=page' ) ); ?><?php _e( 'Add a new page' ); ?><?php if ( $is_block_theme ) : ?><?php _e( 'Customize your entire site with block themes' ); ?><?php _e( 'Design everything on your site &#8212; from the header down to the footer, all using blocks and patterns.' ); ?><?php echo esc_url( admin_url( 'site-editor.php' ) ); ?><?php _e( 'Open site editor' ); ?><?php else : ?><?php _e( 'Start Customizing' ); ?><?php _e( 'Configure your site&#8217;s logo, header, menus, and more in the Customizer.' ); ?><?php if ( $can_customize ) : ?><?php echo wp_customize_url(); ?><?php _e( 'Open the Customizer' ); ?><?php endif; ?><?php endif; ?><?php if ( $is_block_theme ) : ?><?php _e( 'Switch up your site&#8217;s look & feel with Styles' ); ?><?php _e( 'Tweak your site, or give it a whole new look! Get creative &#8212; how about a new color palette or font?' ); ?><?php else : ?><?php _e( 'Discover a new way to build your site.' ); ?><?php _e( 'There is a new kind of WordPress theme, called a block theme, that lets you build the site you&#8217;ve always wanted &#8212; with blocks and styles.' ); ?><?php echo esc_url( __( 'https://wordpress.org/documentation/article/block-themes/' ) ); ?><?php _e( 'Learn about block themes' ); ?><?php endif; ?>