<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="casetwo.master" CodeBehind="ShortAsk.aspx.vb" Inherits="hcmpc.ShortAsk" %>
		...
		<div class="section sectionMain recentNews" id="2186697">
			<div class="primaryContent leftDate">
				<h2 class="subHeading">
					...
					<div class="newsTitle"><%= r("QuestionText")%></div>
				</h2>
				...
				<div class="messageContent baseHtml">
					<div class="postedBy">
						<span class="posted"><div class="sticky"></div>Hỏi Lúc <%=ToVietDate(r("DateAsked"))%>
                        <br />
                        Trả Lời Lúc <%=ToVietDate(r("DateAsked"))%>
                        </span>						
					</div>

					<div class="newsText">                      
                     <div align="center"><i></i></div><div align="center"></div> <%=r("AnsweredText")%></div>
	...
	<nav>   
        <% 
            ...
            %>
             ...
             <% If curSelect > 1 Then%>                   
             <a href="?page=1&ask=<%=id%>" class="text">&lt;&lt;Trang Đầu</a>
                <a href="?page=<%=curSelect-1 %>&ask=<%=id%>" class="text">&lt;Trang Trước</a>
            <%
            End If
            For i As Integer = startPage To endPage
                If i = curSelect Then%>
                    <a href="?page=<%=i%>&ask=<%=id%>" class="currentPage"><%=i%></a>    
                    <%
                    Else
                        %>
                        <a href="?page=<%=i%>&ask=<%=id%>"  rel="start"><%=i%></a>    
                        <%
                    End If
                %>
        ...
        <% If curSelect < numPage Then%>                   
            <a href="?page=<%=curSelect+1 %>&ask=<%=id%>" class="text">Trang Sau &gt;</a>
            <a href="?page=<%=numPage%>&ask=<%=id%>" class="text">Trang Cuối &gt;&gt;</a>	
        <%End If%>
