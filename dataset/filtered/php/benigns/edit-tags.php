...
?><?php if ( $message ) : ?><?php echo $class; ?><?php echo $message; ?><?php
	$_SERVER['REQUEST_URI'] = remove_query_arg( array( 'message', 'error' ), $_SERVER['REQUEST_URI'] );
endif;
?><?php echo esc_attr( $taxonomy ); ?><?php echo esc_attr( $post_type ); ?><?php $wp_list_table->search_box( $tax->labels->search_items, 'tag' ); ?><?php
...
if ( $can_edit_terms ) {
	...
	?><?php echo $tax->labels->add_new_item; ?><?php
	/**
	...
	?><?php echo esc_attr( $current_screen->id ); ?><?php echo esc_attr( $taxonomy ); ?><?php echo esc_attr( $post_type ); ?><?php wp_nonce_field( 'add-tag', '_wpnonce_add-tag' ); ?><?php _ex( 'Name', 'term name' ); ?><?php echo $tax->labels->name_field_description; ?><?php _e( 'Slug' ); ?><?php echo $tax->labels->slug_field_description; ?><?php if ( is_taxonomy_hierarchical( $taxonomy ) ) : ?><?php echo esc_html( $tax->labels->parent_item ); ?><?php
		$dropdown_args = array(
			'hide_empty'       => 0,
			'hide_if_empty'    => false,
			'taxonomy'         => $taxonomy,
		...
		?><?php if ( 'category' === $taxonomy ) : ?><?php _e( 'Categories, unlike tags, can have a hierarchy. You might have a Jazz category, and under that have children categories for Bebop and Big Band. Totally optional.' ); ?><?php else : ?><?php echo $tax->labels->parent_field_description; ?><?php endif; ?><?php endif; // is_taxonomy_hierarchical() ?><?php _e( 'Description' ); ?><?php echo $tax->labels->desc_field_description; ?><?php
	...
	?><?php submit_button( $tax->labels->add_new_item, 'primary', 'submit', false ); ?><?php
	if ( 'category' === $taxonomy ) {
		/**
	...
	?><?php } ?><?php $wp_list_table->views(); ?><?php echo esc_attr( $taxonomy ); ?><?php echo esc_attr( $post_type ); ?><?php $wp_list_table->display(); ?><?php if ( 'category' === $taxonomy ) : ?><?php
	printf(
