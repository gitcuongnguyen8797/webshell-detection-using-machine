...
<%
'========================================================================================
' 文 件 名: admin_errlist.asp
' 版    本: 岭峰网行业专用留言系统2.01.0 (LFMessS 2018 - 2.01.0.F Build 180123)
' 说    明: 非法登陆信息纪录文件 | 请勿自行修改该处文件，以确保程序正确的执行
' 文件日期: 2018-01-23
' 设 计 者: 疾风之狼
'========================================================================================
' 郑州文达岭峰网络科技有限公司.
...
sql="delete * from err where DateDiff('d',addtime,#" & sj & "#)>" & LingFengNet_MS_errlist & ""
conn.Execute(sql)

sql = "Select * FROM err Order By addtime Desc"
Set rs = Server.CreateObject("ADODB.Recordset")
Rs.open sql,conn,1,1
total  = rs.RecordCount
%>
<head>
<title><%= LingFengNet_MS_language_title_errlist %></title>
<%  
  call htmlmeta() 
%> 
...
if len(page)>LingFengNet_MS_pagemax then response.redirect "errlist"&Get_Suffix() 
    ...
    <tr> 
      <td class="bgc2"> 
        <table width="100%" border="0" cellspacing="1" cellpadding="3">
          <tr> 
            <td width="25%" rowspan="5" align="left" valign="top" class="bgc"> &ensp;<%= LingFengNet_MS_language_errlist_mes_1 %> <b><%=i+(page-1)*rs.pagesize%></b> 
              <%= LingFengNet_MS_language_errlist_mes_2 %><br>
            </td>
            <td height="20" align="left" class="bgc"><%= LingFengNet_MS_language_errlist_mes_3 %><%=rs("addtime")%> </td>
          </tr>
          <tr> 
            <td width="75%" height="20" align="left" class="bgc1"><%= LingFengNet_MS_language_errlist_mes_4 %><a href="http://www.lfmesss.com/lfmesss/ip/index.asp?ip=<%=rs("ip")%>&search=<%=rs("ip")%>" target="_blank"><%= LingFengNet_MS_language_errlist_mes_7 %>&ensp;<%=rs("ip")%>&ensp;<%= LingFengNet_MS_language_errlist_mes_8 %></a></td>
          </tr>
          <tr> 
            <td width="75%" height="20" align="left" class="bgc"><%= LingFengNet_MS_language_errlist_mes_5 %><%=rs("errname")%> 
            </td>
          </tr>
          <tr> 
            <td width="75%" height="20" align="left" class="bgc1"><%= LingFengNet_MS_language_errlist_mes_6 %><%=rs("errpass")%></td>
          </tr>
        </table>
      </td>
    </tr>
