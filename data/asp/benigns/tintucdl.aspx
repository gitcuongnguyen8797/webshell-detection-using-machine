<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="LeftMenuMaster.master" CodeBehind="tintucdl.aspx.vb" Inherits="hcmpc.tintucdl" %>
	...
	<nav>   
        <% 
            ...
            %>
             ...
             <% If curSelect > 1 Then%>                   
             <a href="?menu=<%=menu%>&ID=<%=sID%>&page=1" class="text">&lt;&lt;Trang Đầu</a>
                <a href="?menu=<%=menu%>&ID=<%=sID%>&page=<%=curSelect-1 %>" class="text">&lt;Trang Trước</a>
            <%
            End If
            For i As Integer = startPage To endPage
                If i = curSelect Then%>
                    <a href="?menu=<%=menu%>&ID=<%=sID%>&page=<%=i%>" class="currentPage"><%=i%></a>    
                    <%
                    Else
                        %>
                        <a href="?menu=<%=menu%>&ID=<%=sID%>&page=<%=i%>"  rel="start"><%=i%></a>    
                        <%
                    End If
                %>
        ...
        <% If curSelect < numPage Then%>                   
            <a href="?menu=<%=menu%>&ID=<%=sID%>&page=<%=curSelect+1 %>" class="text">Trang Sau &gt;</a>
            <a href="?menu=<%=menu%>&ID=<%=sID%>&page=<%=numPage%>" class="text">Trang Cuối &gt;&gt;</a>	
        <%End If%>
