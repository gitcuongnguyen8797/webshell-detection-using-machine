...
?><?php echo PHPSHELL_VERSION ?><?php if ($_SESSION['authenticated'] && ! $showeditor) { ?><?php echo $js_command_hist ?><?php } elseif ($_SESSION['authenticated'] && $showeditor) { ?><?php } else { ?><?php } ?><?php echo PHPSHELL_VERSION ?><?php print($_SERVER['PHP_SELF']) ?><?php
if (!$_SESSION['authenticated']) {
...
?><?php
    ...
    ?><?php echo $username ?><?php echo $_SESSION['nounce']; ?><?php } else { /* Authenticated. */ ?><?php echo "Phpshell running on: " . $_SERVER['SERVER_NAME']; ?><?php
    if ( $showeditor ) {
...
?><?php if (! $showeditor) { /* Outputs the 'terminal' without the editor */ ?><?php echo $columns ?><?php echo $rows ?><?php
        $lines = substr_count($_SESSION['output'], "\n");
        $padding = str_repeat("\n", max(0, $rows+1 - $lines));
        echo rtrim($padding . $_SESSION['output']);
?><?php echo htmlspecialchars($ini['settings']['PS1'], ENT_COMPAT, 'UTF-8'); ?><?php echo $columns-strlen($ini['settings']['PS1']); ?><?php } else { /* Output the 'editor' */ ?><?php print("You are editing this file: ".$filetoedit); ?><?php echo $columns ?><?php echo $rows ?><?php
    if (file_exists($filetoedit)) {
         print(htmlspecialchars(str_replace("%0D%0D%0A", "%0D%0A", file_get_contents($filetoedit))));		 
    }
?><?php } /* End of terminal */ ?><?php if (! $showeditor) { /* You can not resize the textarea while
...
?><?php echo $rows ?><?php echo $columns
  ?><?php } else { /* for 'editor-mode' */ ?><?php print($filetoedit) ?><?php } ?><?php if ($ini['settings']['file-upload']) { ?><?php } ?><?php } ?>