...
const emotImagePath="images/emot/"
...
strContent=re.Replace(strContent,"<a onfocus=this.blur() href=""$1://$2"" target=_blank><IMG SRC=""$1://$2"" border=0 alt=按此在新窗口浏览图片 onload=""javascript:if(screen.width>800){ if(this.width>screen.width-585)this.width=screen.width-585;}else{ if(this.width>screen.width-360)this.width=screen.width-360}""></a>")
re.Pattern="\[UPLOAD=(gif|jpg|jpeg|bmp|png)\](.[^\[]*)(gif|jpg|jpeg|bmp|png)\[\/UPLOAD\]"
strContent=re.Replace(strContent,"<br><IMG SRC="""&ImagePath&"$1.gif"" border=0>此主题相关图片如下：<br><A HREF=""$2$1"" TARGET=_blank><IMG SRC=""$2$1"" border=0 alt=按此在新窗口浏览图片 onload=""javascript:if(screen.width>800){ if(this.width>screen.width-585)this.width=screen.width-585;}else{ if(this.width>screen.width-360)this.width=screen.width-360}""></A>")
...
'strContent = re.Replace(strContent,"<img align=absmiddle src=images/url.gif border=0><a target=_blank href=""mailto:$2"">$2</a>")
