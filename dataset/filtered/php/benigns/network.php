...
function network_step1( $errors = false ) {
	...
	?><?php _e( 'Welcome to the Network installation process!' ); ?><?php _e( 'Fill in the information below and you&#8217;ll be on your way to creating a network of WordPress sites. Configuration files will be created in the next step.' ); ?><?php
	...
	if ( allow_subdomain_install() && allow_subdirectory_install() ) :
		?><?php esc_html_e( 'Addresses of Sites in your Network' ); ?><?php _e( 'Please choose whether you would like sites in your WordPress network to use sub-domains or sub-directories.' ); ?><?php _e( 'You cannot change this later.' ); ?><?php _e( 'You will need a wildcard DNS record if you are going to use the virtual host (sub-domain) functionality.' ); ?><?php // @todo Link to an MS readme? ?><?php checked( $subdomain_install ); ?><?php _e( 'Sub-domains' ); ?><?php
				printf(
					/* translators: 1: Host name. */
					_x( 'like <code>site1.%1$s</code> and <code>site2.%1$s</code>', 'subdomain examples' ),
					$hostname
				);
				?><?php checked( ! $subdomain_install ); ?><?php _e( 'Sub-directories' ); ?><?php
				printf(
					/* translators: 1: Host name. */
	...
	if ( $is_www ) :
		?><?php esc_html_e( 'Server Address' ); ?><?php
		printf(
		...
		?><?php esc_html_e( 'Server Address' ); ?><?php
				...
				?><?php endif; ?><?php esc_html_e( 'Network Details' ); ?><?php if ( 'localhost' === $hostname ) : ?><?php esc_html_e( 'Sub-directory Installation' ); ?><?php
					printf(
				...
				?><?php elseif ( ! allow_subdomain_install() ) : ?><?php esc_html_e( 'Sub-directory Installation' ); ?><?php
					_e( 'Because your installation is in a directory, the sites in your WordPress network must use sub-directories.' );
					// Uh oh:
				if ( ! allow_subdirectory_install() ) {
					echo ' <strong>' . __( 'Warning:' ) . ' ' . __( 'The main site in a sub-directory installation will need to use a modified permalink structure, potentially breaking existing links.' ) . '</strong>';
				}
				?><?php elseif ( ! allow_subdirectory_install() ) : ?><?php esc_html_e( 'Sub-domain Installation' ); ?><?php
				...
				?><?php endif; ?><?php if ( ! $is_www ) : ?><?php esc_html_e( 'Server Address' ); ?><?php
					printf(
						/* translators: %s: Host name. */
						__( 'The internet address of your network will be %s.' ),
						'<code>' . $hostname . '</code>'
					);
					?><?php endif; ?><?php esc_html_e( 'Network Title' ); ?><?php echo esc_attr( $site_name ); ?><?php _e( 'What would you like to call your network?' ); ?><?php esc_html_e( 'Network Admin Email' ); ?><?php echo esc_attr( $admin_email ); ?><?php _e( 'Your email address.' ); ?><?php submit_button( __( 'Install' ), 'primary', 'submit' ); ?><?php
}
...
function network_step2( $errors = false ) {
	...
	if ( $_POST ) {
		...
		if ( is_multisite() ) {
			...
			?><?php _e( 'The original configuration steps are shown here for reference.' ); ?><?php
		} else {
			$subdomain_install = (bool) $wpdb->get_var( "SELECT meta_value FROM $wpdb->sitemeta WHERE site_id = 1 AND meta_key = 'subdomain_install'" );
			?><?php _e( 'Warning:' ); ?><?php _e( 'An existing WordPress network was detected.' ); ?><?php _e( 'Please complete the configuration steps. To create a new network, you will need to empty or remove the network database tables.' ); ?><?php
		}
	}
	...
	if ( $_POST || ! is_multisite() ) {
		?><?php esc_html_e( 'Enabling the Network' ); ?><?php _e( 'Complete the following steps to enable the features for creating a network of sites.' ); ?><?php
	...
	?><?php
		...
		?><?php
			...
			?><?php echo $subdomain_install ? 'true' : 'false'; ?><?php echo $hostname; ?><?php echo $base; ?><?php
		$keys_salts = array(
			'AUTH_KEY'         => '',
			'SECURE_AUTH_KEY'  => '',
			'LOGGED_IN_KEY'    => '',
		...
		if ( ! empty( $keys_salts ) ) {
			...
			?><?php _e( 'To make your installation more secure, you should also add:' ); ?><?php _e( 'Network configuration authentication keys' ); ?><?php echo $num_keys_salts; ?><?php echo esc_textarea( $keys_salts_str ); ?><?php
		}
		?><?php
	if ( iis7_supports_permalinks() ) :
		...
		$web_config_file = '<?xml version="1.0" encoding="UTF-8"?><?php
				printf(
					/* translators: %s: File name (wp-config.php, .htaccess or web.config). */
					__( 'Network configuration rules for %s' ),
					'<code>web.config</code>'
				);
				?><?php echo esc_textarea( $web_config_file ); ?><?php
...
EOF;
		...
		?><?php
				...
				?><?php echo substr_count( $htaccess_file, "\n" ) + 1; ?><?php echo esc_textarea( $htaccess_file ); ?><?php
	endif; // End IIS/Nginx/Apache code branches.

	if ( ! is_multisite() ) {
		?><?php _e( 'Once you complete these steps, your network is enabled and configured. You will have to log in again.' ); ?><?php echo esc_url( wp_login_url() ); ?><?php _e( 'Log In' ); ?>