...
function media_send_to_editor( $html ) {
	?><?php echo wp_json_encode( $html ); ?><?php
	exit;
}
...
function wp_iframe( $content_func, ...$args ) {
	...
	?><?php bloginfo( 'name' ); ?><?php _e( 'Uploads' ); ?><?php _e( 'WordPress' ); ?><?php
	...
	?><?php echo esc_js( admin_url( 'admin-ajax.php', 'relative' ) ); ?><?php echo (int) is_rtl(); ?><?php
...
function media_upload_form( $errors = null ) {
	...
	*/
	...
	?><?php echo $large_size_h; ?><?php echo $large_size_w; ?><?php echo wp_json_encode( $plupload_init ); ?><?php
	/**
	...
	?><?php _e( 'Drop files to upload' ); ?><?php _ex( 'or', 'Uploader: Drop files here - or - Select Files' ); ?><?php esc_attr_e( 'Select Files' ); ?><?php
	/**
	...
	?><?php
			...
			_e( 'Upload' );
			?><?php submit_button( __( 'Upload' ), 'primary', 'html-upload', false ); ?><?php _e( 'Cancel' ); ?><?php
	/**
...
function media_upload_type_form( $type = 'file', $errors = null, $id = null ) {
	...
	?><?php echo esc_url( $form_action_url ); ?><?php echo $form_class; ?><?php echo $type; ?><?php submit_button( '', 'hidden', 'save', false ); ?><?php echo (int) $post_id; ?><?php wp_nonce_field( 'media-form' ); ?><?php _e( 'Add media files from your computer' ); ?><?php media_upload_form( $errors ); ?><?php
	...
	?><?php submit_button( __( 'Save all changes' ), '', 'save', false ); ?><?php
}
...
function media_upload_type_url_form( $type = null, $errors = null, $id = null ) {
	...
	?><?php echo esc_url( $form_action_url ); ?><?php echo $form_class; ?><?php echo $type; ?><?php echo (int) $post_id; ?><?php wp_nonce_field( 'media-form' ); ?><?php _e( 'Insert media from another website' ); ?><?php
		/** This filter is documented in wp-admin/includes/media.php */
		if ( ! apply_filters( 'disable_captions', '' ) ) {
			?><?php
		}

		?><?php echo esc_url( admin_url( 'images/no.png' ) ); ?><?php echo esc_url( admin_url( 'images/yes.png' ) ); ?><?php echo esc_url( admin_url( 'images/spinner-2x.gif' ) ); ?><?php
	/**
...
function media_upload_gallery_form( $errors ) {
	...
	?><?php _e( 'All Tabs:' ); ?><?php _e( 'Show' ); ?><?php _e( 'Hide' ); ?><?php _e( 'Sort Order:' ); ?><?php _e( 'Ascending' ); ?><?php _e( 'Descending' ); ?><?php _ex( 'Clear', 'verb' ); ?><?php echo esc_url( $form_action_url ); ?><?php echo $form_class; ?><?php wp_nonce_field( 'media-form' ); ?><?php _e( 'Media' ); ?><?php _e( 'Order' ); ?><?php _e( 'Actions' ); ?><?php add_filter( 'attachment_fields_to_edit', 'media_post_single_attachment_fields_to_edit', 10, 2 ); ?><?php echo get_media_items( $post_id, $errors ); ?><?php
		submit_button(
			__( 'Save all changes' ),
			'savebutton',
			'save',
		...
		?><?php echo (int) $post_id; ?><?php echo esc_attr( $GLOBALS['type'] ); ?><?php echo esc_attr( $GLOBALS['tab'] ); ?><?php _e( 'Gallery Settings' ); ?><?php _e( 'Link thumbnails to:' ); ?><?php _e( 'Image File' ); ?><?php _e( 'Attachment Page' ); ?><?php _e( 'Order images by:' ); ?><?php _e( 'Menu order' ); ?><?php _e( 'Title' ); ?><?php _e( 'Date/Time' ); ?><?php _e( 'Random' ); ?><?php _e( 'Order:' ); ?><?php _e( 'Ascending' ); ?><?php _e( 'Descending' ); ?><?php _e( 'Gallery columns:' ); ?><?php esc_attr_e( 'Insert gallery' ); ?><?php esc_attr_e( 'Update gallery settings' ); ?><?php
}
...
function media_upload_library_form( $errors ) {
	...
	?><?php echo esc_attr( $type ); ?><?php echo esc_attr( $tab ); ?><?php echo (int) $post_id; ?><?php echo isset( $_GET['post_mime_type'] ) ? esc_attr( $_GET['post_mime_type'] ) : ''; ?><?php echo isset( $_GET['context'] ) ? esc_attr( $_GET['context'] ) : ''; ?><?php
			/* translators: Hidden accessibility text. */
			echo __( 'Search Media' ) . ':';
			?><?php the_search_query(); ?><?php submit_button( __( 'Search Media' ), '', '', false ); ?><?php
		$type_links = array();
		...
		if ( $month_count && ! ( 1 == $month_count && 0 == $arc_result[0]->mmonth ) ) {
			?><?php selected( $selected_month, 0 ); ?><?php _e( 'All dates' ); ?><?php
			...
			?><?php } ?><?php submit_button( __( 'Filter &#187;' ), '', 'post-query-submit', false ); ?><?php echo esc_url( $form_action_url ); ?><?php echo $form_class; ?><?php wp_nonce_field( 'media-form' ); ?><?php add_filter( 'attachment_fields_to_edit', 'media_post_single_attachment_fields_to_edit', 10, 2 ); ?><?php echo get_media_items( null, $errors ); ?><?php submit_button( __( 'Save all changes' ), 'savebutton', 'save', false ); ?><?php echo (int) $post_id; ?><?php
}
...
function media_upload_html_bypass() {
	?><?php _e( 'You are using the browser&#8217;s built-in file uploader. The WordPress uploader includes multiple file selection and drag and drop capability. <a href="#">Switch to the multi-file uploader</a>.' ); ?><?php
}
...
function edit_form_image_editor( $post ) {
	...
	if ( wp_attachment_is_image( $post->ID ) ) :
		...
		?><?php echo $attachment_id; ?><?php echo $open_style; ?><?php echo $attachment_id; ?><?php echo $attachment_id; ?><?php echo set_url_scheme( $thumb_url[0] ); ?><?php echo $image_edit_button; ?><?php echo $not_open_style; ?><?php echo $attachment_id; ?><?php
	...
	elseif ( isset( $thumb_url[0] ) ) :
		?><?php echo $attachment_id; ?><?php echo $attachment_id; ?><?php echo set_url_scheme( $thumb_url[0] ); ?><?php
	...
	?><?php if ( 'image' === substr( $post->post_mime_type, 0, 5 ) ) : ?><?php _e( 'Alternative Text' ); ?><?php echo esc_attr( $alt_text ); ?><?php
		...
		?><?php endif; ?><?php _e( 'Caption' ); ?><?php echo $post->post_excerpt; ?><?php
	...
	?><?php _e( 'Description' ); ?><?php

	if ( preg_match( '#^(audio|video)/#', $post->post_mime_type ) ) {
		echo ': ' . __( 'Displayed on attachment pages.' );
	}

	?><?php wp_editor( format_to_edit( $post->post_content ), 'attachment_content', $editor_args ); ?><?php
...
function attachment_submitbox_metadata() {
	...
	?><?php if ( $uploaded_by_link ) { ?><?php _e( 'Uploaded by:' ); ?><?php echo $uploaded_by_link; ?><?php echo $uploaded_by_name; ?><?php } else { ?><?php _e( 'Uploaded by:' ); ?><?php echo $uploaded_by_name; ?><?php } ?><?php
	if ( $post->post_parent ) {
		...
		if ( $post_parent ) {
			...
			?><?php if ( $uploaded_to_link ) { ?><?php _e( 'Uploaded to:' ); ?><?php echo $uploaded_to_link; ?><?php echo $uploaded_to_title; ?><?php } else { ?><?php _e( 'Uploaded to:' ); ?><?php echo $uploaded_to_title; ?><?php } ?><?php
		}
	}
	?><?php _e( 'File URL:' ); ?><?php echo esc_attr( $att_url ); ?><?php _e( 'Copy URL to clipboard' ); ?><?php _e( 'Copied!' ); ?><?php echo esc_attr( $att_url ); ?><?php _e( 'Download file' ); ?><?php _e( 'File name:' ); ?><?php echo $filename; ?><?php _e( 'File type:' ); ?><?php
	...
	if ( ! empty( $file_size ) ) {
		?><?php _e( 'File size:' ); ?><?php echo size_format( $file_size ); ?><?php
	}
	...
	if ( preg_match( '#^(audio|video)/#', $post->post_mime_type ) ) {
		...
		foreach ( $fields as $key => $label ) {
			...
			?><?php echo sanitize_html_class( $key ); ?><?php echo $label; ?><?php
		...
		foreach ( $audio_fields as $key => $label ) {
			...
			?><?php echo sanitize_html_class( $key ); ?><?php echo $label; ?><?php echo esc_html( $meta['audio'][ $key ] ); ?><?php
		}
	}

	if ( $media_dims ) {
		?><?php _e( 'Dimensions:' ); ?><?php echo $media_dims; ?><?php
	}

	if ( ! empty( $meta['original_image'] ) ) {
		?><?php _e( 'Original image:' ); ?><?php echo esc_url( wp_get_original_image_url( $attachment_id ) ); ?><?php echo esc_html( wp_basename( wp_get_original_image_path( $attachment_id ) ) ); ?>