...
<br>
<table width="100%" border="0" cellpadding="3" cellspacing="1" bgcolor="#6298E1">
  <form action="?Result=Administrators" method="post" name="formDel" >
    <tr>
      <td width="50" nowrap background="images/th_bg.gif"><font color="#FFFFFF"><strong>序号 </strong></font></td>
      <td width="50" height="24" nowrap background="images/th_bg.gif"><strong><font color="#FFFFFF">生效</font></strong></td>
      <td width="64" nowrap background="images/th_bg.gif"><font color="#FFFFFF"><strong>登录名</strong></font></td>
      <td width="52" nowrap background="images/th_bg.gif"><font color="#FFFFFF"><strong>用户名</strong></font></td>
      <td nowrap background="images/th_bg.gif"><strong><font color="#FFFFFF">操作权限</font></strong></td>
      <td width="118" nowrap background="images/th_bg.gif"><font color="#FFFFFF"><strong><font color="#FFFFFF">最近登录时间</font></strong></font></td>
      <td width="94" nowrap background="images/th_bg.gif"><strong><font color="#FFFFFF">最近登录IP</font></strong></td>
      <td width="118" nowrap background="images/th_bg.gif"><strong><font color="#FFFFFF"><strong>创建时间</strong></font></strong></td>
      <td width="50" nowrap background="images/th_bg.gif"><strong><font color="#FFFFFF">操作</font></strong></td>
      <td width="50" nowrap background="images/th_bg.gif"><input onClick="CheckOthers(this.form)" name="buttonOtherSelect" type="button" class="button"  id="submitOtherSelect" value="选择" style="HEIGHT: 18px;WIDTH: 36px;"></td>
    </tr>
	<% AdminList() %>
  </form>
</table>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      if rs("working") then
        Response.Write "<td nowrap><font color='red'>是</font></td>" & vbCrLf
      else
        Response.Write "<td nowrap><font color='blue'>否</font></td>" & vbCrLf
	  end if
      ...
      Response.Write "<td width='38' nowrap><a href='Admin_Edit.asp?Result=Modify&ID="&rs("ID")&"'><font color='#330099'>修改</font></a></td>" & vbCrLf
    ...
    wend
    Response.Write "<tr>" & vbCrLf
    Response.Write "<td colspan='10' nowrap  bgcolor='#EBF2F9'  align='right'><input name='submitDelSelect' type='submit' class='button'  id='submitDelSelect' value='删除所选' onClick='ConfirmDel(""您真的要删除这些网站管理员吗？"");'></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
...
if request("Result") = "Administrators" then 
	SelectID=request("SelectID")
    if SelectID<>"" then  conn.execute "delete from zhi_rui_s_manage where id in ("&SelectID&")"
	Response.write"<script>location.href=""Admin_List.asp"";</script>"
end if
%>