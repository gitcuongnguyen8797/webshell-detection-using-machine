...
function wp_print_theme_file_tree( $tree, $level = 2, $size = 1, $index = 1 ) {
	...
	if ( is_array( $tree ) ) {
		...
		foreach ( $tree as $label => $theme_file ) :
			...
			?><?php echo esc_attr( $level ); ?><?php echo esc_attr( $size ); ?><?php echo esc_attr( $index ); ?><?php echo esc_html( $label ); ?><?php
					/* translators: Hidden accessibility text. */
					_e( 'folder' );
					?><?php wp_print_theme_file_tree( $theme_file, $level + 1, $index, $size ); ?><?php
		endforeach;
	} else {
		$filename = $tree;
		$url      = add_query_arg(
		...
		?><?php echo esc_attr( $relative_file === $filename ? 'current-file' : '' ); ?><?php echo esc_attr( $relative_file === $filename ? '0' : '-1' ); ?><?php echo esc_url( $url ); ?><?php echo esc_attr( $level ); ?><?php echo esc_attr( $size ); ?><?php echo esc_attr( $index ); ?><?php
	...
	if ( is_array( $tree ) ) {
		...
		foreach ( $tree as $label => $plugin_file ) :
			...
			?><?php echo esc_attr( $level ); ?><?php echo esc_attr( $size ); ?><?php echo esc_attr( $index ); ?><?php echo esc_html( $label ); ?><?php
					/* translators: Hidden accessibility text. */
					_e( 'folder' );
					?><?php wp_print_plugin_file_tree( $plugin_file, '', $level + 1, $index, $size ); ?><?php
		endforeach;
	} else {
		$url = add_query_arg(
			array(
		...
		?><?php echo esc_attr( $file === $tree ? 'current-file' : '' ); ?><?php echo esc_attr( $file === $tree ? '0' : '-1' ); ?><?php echo esc_url( $url ); ?><?php echo esc_attr( $level ); ?><?php echo esc_attr( $size ); ?><?php echo esc_attr( $index ); ?><?php
				if ( $file === $tree ) {
...
function admin_color_scheme_picker( $user_id ) {
	...
	?><?php
		...
		foreach ( $_wp_admin_css_colors as $color => $color_info ) :
			...
			?><?php echo ( $color === $current_color ) ? 'selected' : ''; ?><?php echo esc_attr( $color ); ?><?php echo esc_attr( $color ); ?><?php checked( $color, $current_color ); ?><?php echo esc_url( $color_info->url ); ?><?php echo esc_attr( wp_json_encode( array( 'icons' => $color_info->icon_colors ) ) ); ?><?php echo esc_attr( $color ); ?><?php echo esc_html( $color_info->name ); ?><?php
					foreach ( $color_info->colors as $html_color ) {
						?><?php echo esc_attr( $html_color ); ?><?php
					}
					?><?php
...
function wp_admin_canonical_url() {
	...
	?><?php echo esc_url( $filtered_url ); ?><?php
}
