...
?><?php if ( $notice ) : ?><?php echo $notice; ?><?php endif; ?><?php if ( $message ) : ?><?php echo $message; ?><?php endif; ?><?php _e( '<strong>Connection lost.</strong> Saving has been disabled until you are reconnected.' ); ?><?php _e( 'This post is being backed up in your browser, just in case.' ); ?><?php
/**
...
?><?php wp_nonce_field( $nonce_action ); ?><?php echo (int) $user_ID; ?><?php echo esc_attr( $form_action ); ?><?php echo esc_attr( $form_action ); ?><?php echo esc_attr( $post->post_author ); ?><?php echo esc_attr( $post_type ); ?><?php echo esc_attr( $post->post_status ); ?><?php echo $referer ? esc_url( $referer ) : ''; ?><?php if ( ! empty( $active_post_lock ) ) { ?><?php echo esc_attr( implode( ':', $active_post_lock ) ); ?><?php
}
...
?><?php echo ( 1 === get_current_screen()->get_columns() ) ? '1' : '2'; ?><?php if ( post_type_supports( $post_type, 'title' ) ) { ?><?php
	/**
	...
	?><?php echo $title_placeholder; ?><?php echo esc_attr( $post->post_title ); ?><?php
	/**
...
if ( post_type_supports( $post_type, 'editor' ) ) {
	...
	?><?php echo $_wp_editor_expand_class; ?><?php
	wp_editor(
		$post->post_content,
		'content',
		array(
...
?><?php if ( ! wp_is_mobile() && post_type_supports( $post_type, 'title' ) && '' === $post->post_title ) : ?><?php endif; ?>