...
<table width="95%" border="0" align="center" cellpadding="0" cellspacing="0" class="list_table">
  <form name="frm" method="post" action="?action=edit">
    ...
    <tr> <td height="30" width=221> <div align="right">欢迎使用：</div></td>
...
response.write("<script>alert(""亲！手机号为数字!""); history.go(-1);</script>")
response.End
End If


IF not isNumeric(request("webqq")) then 
response.write("<script>alert(""亲！qq号为数字!""); history.go(-1);</script>")
response.End
End If
