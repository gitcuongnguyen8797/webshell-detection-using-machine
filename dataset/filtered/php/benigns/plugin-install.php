...
function install_dashboard() {
	...
	?><?php _e( 'Popular tags' ); ?><?php _e( 'You may also browse based on the most popular tags in the Plugin Directory:' ); ?><?php
...
function install_search_form( $deprecated = true ) {
	...
	?><?php
			...
			?><?php selected( 'term', $type ); ?><?php _e( 'Keyword' ); ?><?php selected( 'author', $type ); ?><?php _e( 'Author' ); ?><?php selected( 'tag', $type ); ?><?php _ex( 'Tag', 'Plugin Installer' ); ?><?php
			/* translators: Hidden accessibility text. */
			_e( 'Search Plugins' );
			?><?php echo esc_attr( $term ); ?><?php esc_attr_e( 'Search plugins...' ); ?><?php submit_button( __( 'Search Plugins' ), 'hide-if-js', false, false, array( 'id' => 'search-submit' ) ); ?><?php
}
...
function install_plugins_upload() {
	?><?php _e( 'If you have a plugin in a .zip format, you may install or update it by uploading it here.' ); ?><?php echo esc_url( self_admin_url( 'update.php?action=upload-plugin' ) ); ?><?php wp_nonce_field( 'plugin-upload' ); ?><?php
			/* translators: Hidden accessibility text. */
			_e( 'Plugin zip file' );
			?><?php submit_button( __( 'Install Now' ), '', 'install-plugin-submit', false ); ?><?php
}
...
function install_plugins_favorites_form() {
	...
	?><?php _e( 'If you have marked plugins as favorites on WordPress.org, you can browse them here.' ); ?><?php _e( 'Your WordPress.org username:' ); ?><?php echo esc_attr( $user ); ?><?php esc_attr_e( 'Get Favorites' ); ?><?php echo esc_attr( wp_create_nonce( $action ) ); ?><?php
}
...
function install_plugin_information() {
	...
	if ( ! empty( $api->banners ) && ( ! empty( $api->banners['low'] ) || ! empty( $api->banners['high'] ) ) ) {
		...
		?><?php echo esc_url( $low ); ?><?php echo esc_url( $high ); ?><?php
	}
	...
	?><?php echo $_tab; ?><?php echo $_with_banner; ?><?php if ( ! empty( $api->version ) ) { ?><?php _e( 'Version:' ); ?><?php echo $api->version; ?><?php } if ( ! empty( $api->author ) ) { ?><?php _e( 'Author:' ); ?><?php echo links_add_target( $api->author, '_blank' ); ?><?php } if ( ! empty( $api->last_updated ) ) { ?><?php _e( 'Last Updated:' ); ?><?php
					/* translators: %s: Human-readable time difference. */
					printf( __( '%s ago' ), human_time_diff( strtotime( $api->last_updated ) ) );
					?><?php } if ( ! empty( $api->requires ) ) { ?><?php _e( 'Requires WordPress Version:' ); ?><?php
					/* translators: %s: Version number. */
					printf( __( '%s or higher' ), $api->requires );
					?><?php } if ( ! empty( $api->tested ) ) { ?><?php _e( 'Compatible up to:' ); ?><?php echo $api->tested; ?><?php } if ( ! empty( $api->requires_php ) ) { ?><?php _e( 'Requires PHP Version:' ); ?><?php
					/* translators: %s: Version number. */
					printf( __( '%s or higher' ), $api->requires_php );
					?><?php } if ( isset( $api->active_installs ) ) { ?><?php _e( 'Active Installations:' ); ?><?php
				...
				?><?php } if ( ! empty( $api->slug ) && empty( $api->external ) ) { ?><?php echo esc_url( __( 'https://wordpress.org/plugins/' ) . $api->slug ); ?><?php _e( 'WordPress.org Plugin Page &#187;' ); ?><?php } if ( ! empty( $api->homepage ) ) { ?><?php echo esc_url( $api->homepage ); ?><?php _e( 'Plugin Homepage &#187;' ); ?><?php } if ( ! empty( $api->donate_link ) && empty( $api->contributors ) ) { ?><?php echo esc_url( $api->donate_link ); ?><?php _e( 'Donate to this plugin &#187;' ); ?><?php } ?><?php if ( ! empty( $api->rating ) ) { ?><?php _e( 'Average Rating' ); ?><?php
			wp_star_rating(
				array(
					'rating' => $api->rating,
					'type'   => 'percent',
		...
		if ( ! empty( $api->ratings ) && array_sum( (array) $api->ratings ) > 0 ) {
			?><?php _e( 'Reviews' ); ?><?php _e( 'Read all reviews on WordPress.org or write your own!' ); ?><?php
			foreach ( $api->ratings as $key => $ratecount ) {
				...
				?><?php
							...
							?><?php echo 92 * $_rating; ?><?php echo number_format_i18n( $ratecount ); ?><?php
			}
		}
		if ( ! empty( $api->contributors ) ) {
			?><?php _e( 'Contributors' ); ?><?php
				...
				?><?php if ( ! empty( $api->donate_link ) ) { ?><?php echo esc_url( $api->donate_link ); ?><?php _e( 'Donate to this plugin &#187;' ); ?><?php } ?><?php } ?>