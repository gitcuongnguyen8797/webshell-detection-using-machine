...
class WP_Posts_List_Table extends WP_List_Table {
	...
	protected function formats_dropdown( $post_type ) {
		...
		?><?php
			...
			?><?php selected( $displayed_post_format, '' ); ?><?php _e( 'All formats' ); ?><?php
			foreach ( $used_post_formats as $used_post_format ) {
				...
				?><?php selected( $displayed_post_format, $slug ); ?><?php echo esc_attr( $slug ); ?><?php echo esc_html( $pretty_name ); ?><?php
			}
			?><?php
	}
	...
	public function single_row( $post, $level = 0 ) {
		...
		?><?php echo $post->ID; ?><?php echo implode( ' ', get_post_class( $classes, $post->ID ) ); ?><?php $this->single_row_columns( $post ); ?><?php
	...
	public function inline_edit() {
		...
		while ( $bulk < 2 ) :
			...
			?><?php echo $bulk ? 'bulk-edit' : 'inline-edit'; ?><?php echo $classes; ?><?php echo $this->get_column_count(); ?><?php echo $bulk ? 'bulk' : 'quick'; ?><?php echo $bulk ? 'bulk' : 'quick'; ?><?php echo $bulk ? __( 'Bulk Edit' ) : __( 'Quick Edit' ); ?><?php if ( post_type_supports( $screen->post_type, 'title' ) ) : ?><?php if ( $bulk ) : ?><?php else : // $bulk ?><?php _e( 'Title' ); ?><?php if ( is_post_type_viewable( $screen->post_type ) ) : ?><?php _e( 'Slug' ); ?><?php endif; // is_post_type_viewable() ?><?php endif; // $bulk ?><?php endif; // post_type_supports( ... 'title' ) ?><?php if ( ! $bulk ) : ?><?php _e( 'Date' ); ?><?php touch_time( 1, 1, 0, 1 ); ?><?php endif; // $bulk ?><?php
				...
				?><?php if ( ! $bulk && $can_publish ) : ?><?php _e( 'Password' ); ?><?php
							/* translators: Between password field and private checkbox on post quick edit interface. */
							_e( '&ndash;OR&ndash;' );
							?><?php _e( 'Private' ); ?><?php endif; ?><?php if ( count( $hierarchical_taxonomies ) && ! $bulk ) : ?><?php foreach ( $hierarchical_taxonomies as $taxonomy ) : ?><?php echo esc_html( $taxonomy->labels->name ); ?><?php echo ( 'category' === $taxonomy->name ) ? 'post_category[]' : 'tax_input[' . esc_attr( $taxonomy->name ) . '][]'; ?><?php echo esc_attr( $taxonomy->name ); ?><?php wp_terms_checklist( 0, array( 'taxonomy' => $taxonomy->name ) ); ?><?php endforeach; // $hierarchical_taxonomies as $taxonomy ?><?php endif; // count( $hierarchical_taxonomies ) && ! $bulk ?><?php
				if ( post_type_supports( $screen->post_type, 'author' ) && $bulk ) {
					echo $authors_dropdown;
				}
				?><?php if ( post_type_supports( $screen->post_type, 'page-attributes' ) ) : ?><?php if ( $post_type_object->hierarchical ) : ?><?php _e( 'Parent' ); ?><?php
							$dropdown_args = array(
							...
							?><?php endif; // hierarchical ?><?php if ( ! $bulk ) : ?><?php _e( 'Order' ); ?><?php echo $post->menu_order; ?><?php endif; // ! $bulk ?><?php endif; // post_type_supports( ... 'page-attributes' ) ?><?php if ( 0 < count( get_page_templates( null, $screen->post_type ) ) ) : ?><?php _e( 'Template' ); ?><?php if ( $bulk ) : ?><?php _e( '&mdash; No Change &mdash;' ); ?><?php endif; // $bulk ?><?php
							/** This filter is documented in wp-admin/includes/meta-boxes.php */
							$default_title = apply_filters( 'default_page_template_title', __( 'Default template' ), 'quick-edit' );
							?><?php echo esc_html( $default_title ); ?><?php page_template_dropdown( '', $screen->post_type ); ?><?php endif; ?><?php if ( count( $flat_taxonomies ) && ! $bulk ) : ?><?php foreach ( $flat_taxonomies as $taxonomy ) : ?><?php if ( current_user_can( $taxonomy->cap->assign_terms ) ) : ?><?php $taxonomy_name = esc_attr( $taxonomy->name ); ?><?php echo esc_html( $taxonomy->labels->name ); ?><?php echo $taxonomy_name; ?><?php echo esc_attr( $taxonomy->name ); ?><?php echo esc_attr( $taxonomy->name ); ?><?php echo esc_attr( $taxonomy->name ); ?><?php echo esc_attr( $taxonomy->name ); ?><?php echo esc_html( $taxonomy->labels->separate_items_with_commas ); ?><?php endif; // current_user_can( 'assign_terms' ) ?><?php endforeach; // $flat_taxonomies as $taxonomy ?><?php endif; // count( $flat_taxonomies ) && ! $bulk ?><?php if ( post_type_supports( $screen->post_type, 'comments' ) || post_type_supports( $screen->post_type, 'trackbacks' ) ) : ?><?php if ( $bulk ) : ?><?php if ( post_type_supports( $screen->post_type, 'comments' ) ) : ?><?php _e( 'Comments' ); ?><?php _e( '&mdash; No Change &mdash;' ); ?><?php _e( 'Allow' ); ?><?php _e( 'Do not allow' ); ?><?php endif; ?><?php if ( post_type_supports( $screen->post_type, 'trackbacks' ) ) : ?><?php _e( 'Pings' ); ?><?php _e( '&mdash; No Change &mdash;' ); ?><?php _e( 'Allow' ); ?><?php _e( 'Do not allow' ); ?><?php endif; ?><?php else : // $bulk ?><?php if ( post_type_supports( $screen->post_type, 'comments' ) ) : ?><?php _e( 'Allow Comments' ); ?><?php endif; ?><?php if ( post_type_supports( $screen->post_type, 'trackbacks' ) ) : ?><?php _e( 'Allow Pings' ); ?><?php endif; ?><?php endif; // $bulk ?><?php endif; // post_type_supports( ... comments or pings ) ?><?php _e( 'Status' ); ?><?php if ( $bulk ) : ?><?php _e( '&mdash; No Change &mdash;' ); ?><?php endif; // $bulk ?><?php if ( $can_publish ) : // Contributors only get "Unpublished" and "Pending Review". ?><?php _e( 'Published' ); ?><?php _e( 'Scheduled' ); ?><?php if ( $bulk ) : ?><?php _e( 'Private' ); ?><?php endif; // $bulk ?><?php endif; ?><?php _e( 'Pending Review' ); ?><?php _e( 'Draft' ); ?><?php if ( 'post' === $screen->post_type && $can_publish && current_user_can( $post_type_object->cap->edit_others_posts ) ) : ?><?php if ( $bulk ) : ?><?php _e( 'Sticky' ); ?><?php _e( '&mdash; No Change &mdash;' ); ?><?php _e( 'Sticky' ); ?><?php _e( 'Not Sticky' ); ?><?php else : // $bulk ?><?php _e( 'Make this post sticky' ); ?><?php endif; // $bulk ?><?php endif; // 'post' && $can_publish && current_user_can( 'edit_others_posts' ) ?><?php if ( $bulk && current_theme_supports( 'post-formats' ) && post_type_supports( $screen->post_type, 'post-formats' ) ) : ?><?php $post_formats = get_theme_support( 'post-formats' ); ?><?php _ex( 'Format', 'post format' ); ?><?php _e( '&mdash; No Change &mdash;' ); ?><?php echo get_post_format_string( 'standard' ); ?><?php if ( is_array( $post_formats[0] ) ) : ?><?php foreach ( $post_formats[0] as $format ) : ?><?php echo esc_attr( $format ); ?><?php echo esc_html( get_post_format_string( $format ) ); ?><?php endforeach; ?><?php endif; ?><?php endif; ?><?php
			...
			?><?php if ( ! $bulk ) : ?><?php wp_nonce_field( 'inlineeditnonce', '_inline_edit', false ); ?><?php _e( 'Update' ); ?><?php else : ?><?php submit_button( __( 'Update' ), 'primary', 'bulk_edit', false ); ?><?php endif; ?><?php _e( 'Cancel' ); ?><?php if ( ! $bulk ) : ?><?php endif; ?><?php echo esc_attr( $m ); ?><?php echo esc_attr( $screen->id ); ?><?php if ( ! $bulk && ! post_type_supports( $screen->post_type, 'author' ) ) : ?><?php echo esc_attr( $post->post_author ); ?><?php endif; ?><?php
			$bulk++;
		endwhile;
		?>