...
<body>
             <div class="box_top">
              <div align="center"><strong>添加企业荣誉</strong></div>
            </div>
...
response.Write("<script language=javascript>alert('有这篇企业荣誉了，为了搜索引擎优化，请不要重名!');history.go(-1)</script>") 
response.End()
end if
end if
...
sj=trim(request("sj"))
if honorname=""  then 
response.Write("<script language=javascript>alert('企业荣誉标题不能为空!');history.go(-1)</script>") 
response.end 
end if
if pic=""  then 
response.Write("<script language=javascript>alert('企业荣誉图片不能为空!');history.go(-1)</script>") 
response.end 
end if
