<%@ Page Language="vb" AutoEventWireup="false" MasterPageFile="casetwo.master" CodeBehind="lienhe.aspx.vb" Inherits="hcmpc.lienhe" %>
...
<script>
   ...
   placeMarker(haight, map,"Tổng Công Ty Điện Lực Tp.HCM - 35 Tôn Đức Thắng, Phường Bến Nghé, Q1");
...
//        markers.markers.push(marker);
    ...
    function getInfo() {
       ...
       placeMarker(result, map,tenCongTy + " - "+ diachi);
    }  
...
<div id="recentNews">
        ...
        <div style="padding-left:48px" class="gtext">
          ...
          <select id="ViTriDonVi" class="posDirect" style="font-size:13px; color:#666; border:1px solid #aaa;" onchange="getInfo();">  
                <% For Each r As DataRow In dt.Rows%> 	
                          <option value="<%=r("lat")%>,<%=r("lng")%>@#$<%=r("DiaChi")%>@#$<%=r("DienThoai")%>@#$<%=r("TenDayDu").ToString().ToUpper()%>"> <%=r("TenDonVi")%> </option>                       
                 <%Next%>
                  </select>        
             </h4>
            <div id="Info">                    
                <div  style="float: left; color:#0066CC; font-weight:bold" id="lbCongTy"><%=dt.Rows(0)("TenDayDu").ToString().ToUpper()%></div>
                <br />              
