...
function _admin_notice_post_locked() {
	...
	if ( $locked ) {
		...
		?><?php echo get_avatar( $user->ID, 64 ); ?><?php
		if ( $override ) {
		...
		?><?php echo esc_url( $sendback ); ?><?php echo $sendback_text; ?><?php if ( $preview_link ) { ?><?php echo $tab_last; ?><?php echo esc_url( $preview_link ); ?><?php _e( 'Preview' ); ?><?php
		}

		// Allow plugins to prevent some users overriding the post lock.
		if ( $override ) {
			?><?php echo esc_url( add_query_arg( 'get-post-lock', '1', wp_nonce_url( get_edit_post_link( $post->ID, 'url' ), 'lock-post_' . $post->ID ) ) ); ?><?php _e( 'Take over' ); ?><?php
		}

		?><?php
	} else {
		?><?php echo esc_url( admin_url( 'images/spinner-2x.gif' ) ); ?><?php _e( 'Saving revision&hellip;' ); ?><?php _e( 'Your latest changes were saved as a revision.' ); ?><?php
			/**
			...
			?><?php echo esc_url( $sendback ); ?><?php echo $sendback_text; ?><?php
	}
...
function the_block_editor_meta_boxes() {
	...
	// Render meta boxes.
	?><?php the_block_editor_meta_box_post_form_hidden_fields( $post ); ?><?php echo esc_url( admin_url( 'post.php' ) ); ?><?php wp_nonce_field( 'toggle-custom-fields', 'toggle-custom-fields-nonce' ); ?><?php foreach ( $locations as $location ) : ?><?php echo esc_attr( $location ); ?><?php
					do_meta_boxes(
						$current_screen,
						$location,
						$post
...
function the_block_editor_meta_box_post_form_hidden_fields( $post ) {
	...
	?><?php echo (int) $user_id; ?><?php echo esc_attr( $form_action ); ?><?php echo esc_attr( $form_action ); ?><?php echo esc_attr( $post->post_type ); ?><?php echo esc_attr( $post->post_status ); ?><?php echo $referer ? esc_url( $referer ) : ''; ?>